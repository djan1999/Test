...
| A2 | EDIT "admin-gated like other admin entry points" | There are **no per-user roles**: ADMIN is a `mode` any authenticated workspace member can enter from the login screen (`isPlatformAdmin` is platform-level workspace switching, not a role gate). | The EDIT toggle on the floor view sits behind the same (only) gate that protects every admin entry point today: workspace auth. Flagged for Djan — if a real role gate lands later, the toggle has one mount point to hide. |
