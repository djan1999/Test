...
| L9 | The DATABASE refuses a worked-content wipe and records every board version. `private.assert_worked_content_shield` rejects a skeleton over worked content unless the writer's synced ancestor held it (`allow_clear` attestation); `service_tables_history` is written by a definer trigger no client can skip, and `restore_service_board` rewinds a service's board to any recorded moment (SYSTEM → Board Time Machine). | `supabase/tests/board_history.sql` (pgTAP — run with `supabase test db`); `serviceTableCas.test.js` (attestation values, predicate lockstep); the whole vitest suite runs against the shield-emulating fake backend, proving no legitimate flow trips it |
...
| L11 | REPLAY PARITY: folding a service's facts (production deriver `boardFactsFromDiff` ∘ production reducer `foldServiceEvents`, server order) reproduces the board's comparable projection — every table, seat, drink, and fire, and the party's name and arrival time (full-fidelity: a wiped name is divergence). The reducer is total and no-throw: unknown event types, junk payloads and missing tables fold as no-ops, so replaying an append-only log can never crash a device. This property is what the Phase-4 source-of-truth flip stands on (`docs/EVENT_LOG_PLAN.md`). | `eventFold.test.js` (150-seeded-night property + resilience); live measurement via SYSTEM → Logbook → CHECK PARITY (`checkLogParity` in App); automatic end-of-night verdicts filed by every service end — `parityRecord.test.js` pins that the recorder can never affect ending a service |
