...
export default function App() {
  ...
  const archiveAndClearAll = async ({ skipConfirm = false } = {}) => {
    ...
    try {
      ...
      // write is idempotent per service instance, so a retry or a second
      // tablet ending the same service adds nothing. It is also deliberately
      // unawaited and unreported: guest history is worth having, and it is
      // never worth holding up the end of a service or showing a waiter an
      // error about it.
      if (RESERVATIONS_LOOP2_ENABLED) {
        // The entity model copies nothing — the ended service IS the archive —
        // so there is no archive row to read any more. The visit reader is
        // handed the service's own id and the board as it stood at the end.
        // svcId being the instance id is what keeps the fold idempotent:
        // ending the same service twice yields the same visitIds rather than a
        // second dinner nobody ate.
        const endedService = {
          id: svcId,
          date,
          state: { date, serviceSession: session, tables: snap?.tables || [] },
        };
        // Captured out here on purpose: inside this function `session` is the
        // SERVICE session ("dinner"), not the Supabase one, so reading
        // session.access_token here would quietly send no credentials at all.
        const credentials = reservationFeedCredentials || undefined;
        void (async () => {
          try {
            ...
            const visits = visitsFromArchive(endedService, projectedBookingsRef.current);
            if (!visits.length) return;
            await recordServiceVisits(visits, credentials);
          } catch (error) {
