...
function generateMenuHTML({ seat, table, menuTitle = "WINTER MENU", teamNames = "", menuCourses = MENU_DATA, beerChoice = null }) {
  ...
  const normalizeCourseKey = (course, i) => {
    const rawKey = String(course?.course_key || "").trim().toLowerCase();
    if (rawKey) return rawKey;
    const rawName = String(course?.menu?.name || "").trim().toLowerCase();
    const nameMap = {
      "sour soup": "sour_soup",
      "linzer eye": "linzer_eye",
      "trout belly": "trout_belly",
      "chamois": "chamois",
      "crayfish": "crayfish",
      "danube salmon": "danube_salmon",
      "beetroot": "beetroot",
      "squash": "squash",
    ...
    return nameMap[rawName] || `course_${i + 1}`;
  };

  const shouldIncludeCourse = ({ courseKey, courseName, optionalFlag }) => {
    const isBeetrootCourse =
      optionalFlag === "beetroot" || courseKey === "beetroot" || courseName === "BEETROOT";
    const isCakeCourse =
      optionalFlag === "cake" || courseKey === "pear" || courseKey === "pear_cake" || courseName === "PEAR";
    const isOptionalCheeseCourse =
      optionalFlag === "cheese" || courseKey === "cheese" || courseName === "CHEESE";

    if (isBeetrootCourse && !hasBeetroot) return false;
    if (isCakeCourse && !hasCake) return false;
    if (isOptionalCheeseCourse && !hasCheese) return false;

    if (!isShort) {
      return true;
    }

    if (courseKey === "beetroot") return hasBeetroot;
    if (courseKey === "pear" || courseKey === "pear_cake") return hasCake;
    if (courseKey === "cheese") return hasCheese;

    return SHORT_MENU_ORDER.includes(courseKey);
  };

  const visibleCourses = [];
  menuCourses.forEach((course, i) => {
    const courseKey = normalizeCourseKey(course, i);
    const courseName = String(course?.menu?.name || "").trim().toUpperCase();
    const optionalFlag = String(course?.optional_flag || "").trim().toLowerCase();

    if (!shouldIncludeCourse({ courseKey, courseName, optionalFlag })) return;

    let orderValue;
    if (isShort) {
      const shortIdx = SHORT_MENU_ORDER.indexOf(courseKey);
      orderValue = shortIdx === -1 ? 9999 : shortIdx;
    } else {
