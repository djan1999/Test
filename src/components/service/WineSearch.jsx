...
export default function WineSearch({ wineObj, wines = [], onChange, placeholder, byGlass = null, compact = false }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", width: "100%" }}>
      {wineObj ? (
        <div style={{ display: "flex", alignItems: "center", border: "1px solid #d8d8d8", borderRadius: 0, padding: `${py}px 28px ${py}px 10px`, background: "#fafafa", position: "relative", fontSize: fs, fontFamily: tokens.font, color: "#4a4a4a" }}>
        ...
        <div style={{ position: "absolute", top: "calc(100% + 3px)", left: 0, right: 0, background: "#fff", border: "1px solid #e8e8e8", borderRadius: 0, zIndex: 200, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", overflow: "hidden" }}>
          {results.map((w) => (
            <div key={w.id} onMouseDown={() => { setQ(""); setOpen(false); onChange(w); }} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #f5f5f5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {w.byGlass && <span style={{ fontFamily: tokens.font, fontSize: 9, letterSpacing: 1, color: "#444", border: "1px solid #e8e8e8", borderRadius: 0, padding: "2px 5px" }}>glass</span>}
            </div>
          ))}
        </div>
      )}
