    ...
    <div ref={ref} style={{ position: "relative", width: "100%" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 3px)", left: 0, right: 0, background: tokens.neutral[0], border: `1px solid ${tokens.neutral[200]}`, borderRadius: 0, zIndex: 200, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", overflow: "hidden" }}>
          {results.map((w) => (
            <div key={w.id} onMouseDown={() => { setQ(""); setOpen(false); onChange(w); }} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: `1px solid ${tokens.neutral[100]}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <span style={{ fontFamily: tokens.font, fontSize: 12, color: tokens.neutral[900] }}>{w.name}</span>
