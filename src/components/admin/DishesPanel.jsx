...
export default function DishesPanel({ dishes, onUpdateDishes }) {
  ...
  return (
    <>
      {/* Main Dishes info */}
      <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 1, color: tokens.ink[3], marginBottom: 8 }}>
        DISHES & RESTRICTIONS
      </div>
      <div style={{
        fontFamily: FONT, fontSize: 10, color: tokens.ink[3], padding: "16px 20px",
        background: tokens.ink.bg, border: `1px solid ${tokens.ink[4]}`, borderRadius: 0, marginBottom: 28,
        lineHeight: 1.6,
      }}>
        Main dish names, descriptions, dietary flags, restriction variants, course keys, and kitchen notes are managed in <strong style={{ color: tokens.ink[2] }}>Menu Layout</strong>.
        <br />
        Use the + button on each course to add restrictions and pairings.
      </div>

      {/* Extra Dishes */}
      <div style={{ fontFamily: FONT, fontSize: 9, letterSpacing: 2, color: tokens.ink[3], marginBottom: 16, textTransform: "uppercase" }}>
        Extra Dishes
      </div>
      <div style={{ fontFamily: FONT, fontSize: 10, color: tokens.ink[3], marginBottom: 16 }}>
              ...
              {dish.pairings.map((p, idx) => (
                <div key={idx} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                  ...
                  {dish.pairings.length > 1 && (
                    <button onClick={() => removePairing(dish.id, idx)} style={{ background: "none", border: "none", color: tokens.ink[2], cursor: "pointer", fontSize: 14, lineHeight: 1, padding: 0 }}>x</button>
                  )}
                </div>
              ))}
