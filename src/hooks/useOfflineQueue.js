  ...
  const flushQueue = useCallback(async () => {
    ...
    try {
      ...
      while (idx < items.length) {
        ...
        let result;
        if (item.op === "upsert") result = await query.upsert(item.payload, item.options || {});
        else if (item.op === "insert") result = await query.insert(item.payload, item.options || {});
        else if (item.op === "update") result = await query.update(item.payload, item.options || {});
        else break;
