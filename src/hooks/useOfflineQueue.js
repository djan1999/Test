...
import { enqueue, readQueue, removeAt, updateAt } from "../lib/syncQueue.js";

// A job that keeps failing while we're online (e.g. a constraint violation)
// must not block the queue forever — drop it after this many flush attempts.
  ...
  const flushQueue = useCallback(async () => {
    ...
    try {
      ...
      while (idx < items.length) {
        ...
        if (result?.error) {
          // Stop flushing (keeps write ordering: later jobs must not land
          // before this one), but count the attempt so a poisoned job is
          // eventually dropped instead of blocking the queue forever.
          const attempts = (item.attempts || 0) + 1;
          if (attempts >= MAX_FLUSH_ATTEMPTS) {
            console.warn("[offline-queue] dropping job after repeated failures:", item.table, item.op, result.error?.message);
            removeAt(idx);
          } else {
            updateAt(idx, { ...item, attempts });
          }
          break;
        }
        removeAt(idx);
