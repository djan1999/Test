...
// workspace membership, so this is no longer used for gating).
...
// Enabled app-wide: on whenever a URL is configured and a workspace is active.
// Per-workspace data isolation is enforced server-side by the sync rules, not
// here. Accounts with no synced rows (e.g. the platform-admin, which isn't a
// workspace member) simply find an empty local DB and fall back to Supabase.
